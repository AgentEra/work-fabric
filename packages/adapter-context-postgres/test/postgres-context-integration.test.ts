...
import { EXCHANGE_AUTHORITY_MIGRATION } from "@work-fabric/adapter-storage-postgres";
...
describe("PostgreSQL context integration", () => {
  it.skipIf(!live)("isolates tenant context metadata with RLS", async () => {
    ...
    const setup = await pool.connect(); await setup.query(`CREATE SCHEMA "${schema}"`); await setup.query(`SET search_path TO "${schema}"`); await runMigrations(setup, [TENANT_CONTEXT_MIGRATION, EXCHANGE_AUTHORITY_MIGRATION, CONTEXT_MIGRATION]); setup.release();
