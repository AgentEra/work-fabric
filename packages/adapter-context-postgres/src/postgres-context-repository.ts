...
function strings(value: JsonValue | undefined, label: string): readonly string[] { if (!Array.isArray(value) || value.some((item) => typeof item !== "string" || item.length === 0 || item.length > 128) || new Set(value).size !== value.length) throw new TypeError(`${label} must be a unique opaque ID array`); return value as string[]; }
function reference(bundle: JsonObject): { reference: ContextReference; actorIds: readonly string[]; endpointIds: readonly string[]; expiresAt: string | null } {
  ...
  const expiresAt = visibility.expires_at === null ? null : (typeof visibility.expires_at === "string" ? visibility.expires_at : (() => { throw new TypeError("visibility_scope.expires_at must be a timestamp or null"); })()); if (expiresAt !== null) parseUtcTimestamp(expiresAt, "visibility_scope.expires_at");
  return { reference: { context_id: contextId, version: bundleVersion, digest: bundleDigest }, actorIds: strings(visibility.actor_ids, "visibility_scope.actor_ids"), endpointIds: strings(visibility.endpoint_ids, "visibility_scope.endpoint_ids"), expiresAt };
}
...
export class PostgresContextRepository implements ContextRepository {
  ...
  async putBundle(tenantId: string, bundle: JsonObject): Promise<ContextReference> {
    ...
    return this.run(tenantId, async (client) => {
      ...
      const inserted = await client.query("INSERT INTO work_fabric_context_bundles (tenant_id,context_id,version,digest,expires_at,bundle,actor_ids,endpoint_ids) VALUES ($1,$2,$3,$4,$5,$6::jsonb,$7::jsonb,$8::jsonb) ON CONFLICT (tenant_id,context_id,version) DO NOTHING", [tenantId, parsed.reference.context_id, parsed.reference.version, parsed.reference.digest, parsed.expiresAt, JSON.stringify(candidate), JSON.stringify(parsed.actorIds), JSON.stringify(parsed.endpointIds)]);
      if (inserted.rowCount === 0) { const replay = await client.query<Record<string, unknown>>("SELECT bundle FROM work_fabric_context_bundles WHERE tenant_id=$1 AND context_id=$2 AND version=$3", [tenantId, parsed.reference.context_id, parsed.reference.version]); const stored = replay.rows[0] === undefined ? null : rowJson<JsonObject>(replay.rows[0].bundle); if (stored === null || !isDeepStrictEqual(stored, candidate)) throw new Error(`Context ${parsed.reference.context_id} version ${parsed.reference.version} is immutable and has a different body`); }
      return clone(parsed.reference);
    });
  }
  ...
  async checkAvailability(request: ContextAccessRequest): Promise<ContextAvailability> {
    ...
    return this.run(candidate.tenant_id, async (client) => {
      const result = await client.query<Record<string, unknown>>("SELECT digest,expires_at,actor_ids,endpoint_ids FROM work_fabric_context_bundles WHERE tenant_id=$1 AND context_id=$2 AND version=$3", [candidate.tenant_id, candidate.reference?.context_id, candidate.reference?.version]);
