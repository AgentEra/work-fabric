...
export interface FeishuPluginOutboundConfig {
  readonly enabled: boolean;
  readonly default_render_mode: "text" | "card";
...
export interface FeishuPluginWorkerConfig {
...
export function validateFeishuPluginConfig(value: unknown): FeishuPluginConfig {
  ...
  const commonInbound = {
    enabled: bool(inbound.enabled, "inbound.enabled"),
    mention_only: true,
...
export function feishuSecretPaths(
  ...
  if (config.inbound.transport === "webhook") {
    const credentials = config.credentials as FeishuWebhookCredentials;
    fields.push("verification_token");
    if (credentials.encrypt_key !== undefined) fields.push("encrypt_key");
  }
