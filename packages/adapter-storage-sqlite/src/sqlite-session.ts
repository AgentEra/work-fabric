...
export class SqliteSession {
  ...
  pragma(name: "foreign_keys" | "busy_timeout" | "journal_mode"): unknown {
    const row = this.prepare(`PRAGMA ${name}`).get() as Record<string, unknown> | undefined;
    return name === "busy_timeout" ? row?.timeout : row?.[name];
  }
