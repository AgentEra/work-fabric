...
function assertTenant(actual: string, expected: string): void {
  if (actual !== expected) throw new Error("tenant context mismatch");
}
...
function decisionValues(record: AdmissionDecisionRecord): readonly (string | number | null)[] {
  ...
  return [
    ...
    evidence === undefined ? 0 : 1,
    evidence?.membership ?? null,
    evidence?.active === undefined || evidence.active === null ? null : evidence.active ? 1 : 0,
...
export class SqliteParticipantBindingStore implements ParticipantBindingStore {
  constructor(private readonly session: SqliteSession, private readonly tenantId: string) {
      ...
      return this.session.transaction(() => {
        ...
        const row = this.session.prepare(`
          SELECT tenant_id, connector_id, source_system, external_tenant_id, external_subject_type,
                 external_subject_fingerprint, actor_id, actor_type, endpoint_id, created_at
          FROM work_fabric_admission_bindings
          WHERE tenant_id = ? AND connector_id = ? AND source_system = ? AND external_tenant_id = ?
            AND external_subject_type = ? AND external_subject_fingerprint = ?
        `).get(
          this.tenantId,
          input.request.connector_id,
          input.request.source_system,
...
export class SqliteAdmissionDecisionStore implements AdmissionDecisionStore {
  constructor(private readonly session: SqliteSession, private readonly tenantId: string) {
    if (typeof tenantId !== "string" || tenantId.length === 0) throw new TypeError("tenantId must be non-empty");
  }
  ...
  async findByIngress(input: Parameters<AdmissionDecisionStore["findByIngress"]>[0]): Promise<AdmissionDecisionRecord | null> {
    try {
      assertTenant(input.tenant_id, this.tenantId);
      const row = this.session.prepare(`
        SELECT * FROM work_fabric_admission_decisions
        WHERE tenant_id = ? AND connector_id = ? AND source_system = ? AND external_tenant_id = ? AND ingress_id = ?
      `).get(this.tenantId, input.connector_id, input.source_system, input.external_tenant_id, input.ingress_id) as DecisionRow | undefined;
