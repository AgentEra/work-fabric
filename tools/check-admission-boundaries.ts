...
function analyzeSourceFile(sourceFile: SourceFile, repositoryPath: string): SourceAnalysis {
  ...
  const visit = (node: Node): void => {
    ...
    if (isImportDeclaration(node)) {
      ...
      if (isObservationCall(node.expression) && node.arguments.some((argument) =>
        subtreeContainsExecutableIdentifier(argument, sensitiveIdentifiers)
      )) sensitiveSink = true;
    }
    ...
    if (consoleSource && subtreeContainsExecutableIdentifier(node, sensitiveIdentifiers)) sensitiveSink = true;
    if (isVariableDeclaration(node)) {
      const name = nodeName(node.name);
      if (name !== undefined && normalizedIdentifier(name).split(" ").includes("decision") &&
        node.initializer !== undefined &&
        subtreeContainsExecutableIdentifier(node.initializer, sensitiveIdentifiers)) sensitiveSink = true;
    }
  ...
  visit(sourceFile);
  return { moduleSpecifiers, policyPrecedence, exchangeApplicationUse, sensitiveSink };
}
...
const forbiddenAdmissionImport = /^(?:@larksuiteoapi\/node-sdk(?:\/|$)|@slack\/|@wecom\/|yaml$|better-sqlite3$|sqlite3$|node:sqlite$|pg$|postgres$|@work-fabric\/(?:wfpp|protocol-runtime|exchange-spi|exchange-core|exchange-runtime|adapter-(?:configuration-yaml|admission-sqlite|admission-postgres|storage-sqlite|storage-postgres))(?:\/|$))/;
...
export async function checkAdmissionBoundaries(root = resolve(".")): Promise<AdmissionBoundaryReport> {
  ...
  try {
    ...
    for (const path of discovery.files) {
      ...
      const analysis = analyzeSourceFile(sourceFile, repositoryPath);
      ...
      if (admissionCore) {
        for (const specifier of analysis.moduleSpecifiers) {
          ...
          if (forbiddenAdmissionImport.test(specifier)) {
