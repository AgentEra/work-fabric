...
22. **语义判断归决策体**：对于 Agent 承接的自然语言工作，意图、上下文依赖、信息充分性、相关性和业务含义必须由 Agent 模型判断；禁止用关键词、正则表达式或固定自然语言词表代替语义决策。确定性代码只校验协议结构、能力契约、Authority、身份、安全边界和资源预算。Fabric、Channel 与 Provider 不得代替 Agent 解释自然语言。
