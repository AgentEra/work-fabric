...
def role_prompt(
    ...
    *,
    ...
    turn_contract = (
        ...
        "A failed result may include retry_after as timing evidence; it does not itself authorize "
        "another capability call. You must not request a new invocation before retry_after. If "
        "retry_after is at or after task.result_due_at, stop and explain that no retry fits the "
        "current Handoff deadline. Otherwise decide semantically whether a later invocation is "
        "still needed and authorized; never treat retryable or retry_after as an automatic retry. "
