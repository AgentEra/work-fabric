...
Actor/Endpoint Target 是明确地址。Capability Requirement Target 表达尚未解析的能力需求，并显式选择一种分派模式：默认 `external_resolution` 由人工、规则服务或 AI Brain 等外部 Target Resolver 提交明确解析结果；`eligible_pool_claim` 则把 Handoff 放入权限与能力过滤的候选池，由外部 Endpoint 主动 Claim。两种模式都不授权 Exchange 内置排名、推荐或业务调度。Exchange 只负责候选事实、资格校验、原子 Claim/Lease、权威记录和可靠派发。

默认 Capability Offer 创建 `target_resolution_pending` Handoff。成功解析后，原始 Capability Requirement 保持不可变，明确 Actor/Endpoint 作为独立 Target Binding 被记录，Handoff 才进入 `offered`。显式候选池 Offer 创建 `claimable` Handoff；Claim 只形成有时限、带 fencing token 的占用，不转移执行责任，只有携带当前 Claim fence 的 Accept 才进入 `accepted`。`target_unavailable` 是透明终态；并发解析或并发 Claim 只允许一个权威结果被提交，这一存储不变量不构成业务调度策略。
...
Canonical Receipt 只能由 Authoritative Exchange 签发。`delivered`、`received`、`claim_acquired`、`responsibility_accepted`、`result_received` 与 `result_verified` 是不同事实。`claim_acquired` 返回当前 Claim ID、fencing token 与 Lease 信息，但明确不代表已接受责任。

成功的 Operation Result 必须返回 Resource Version；只有生命周期明确签发 Receipt 的交互才返回非空 `receipt`。当前 Handoff Core 在 Claim、Accept、Return Result 与 Verify 成功时签发对应 Receipt，Offer、Claim Renew/Release/Expire、Status、Close、Cancel 等成功交互必须显式返回 `receipt: null`，不得虚构 Receipt。
