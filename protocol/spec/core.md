...
Actor/Endpoint Target 是明确地址。Capability Requirement Target 表达尚未解析的能力需求，不授权 Exchange 内置选择接收方；人工、规则服务或 AI Brain 等外部 Target Resolver 通过协议提交明确解析结果。Exchange 负责候选事实、目标资格校验、权威记录和后续可靠派发，不负责匹配、排名、推荐或执行计划。Target Resolution 的规范消息与生命周期扩展将在 Capability Target 对外 Binding 前补齐。
